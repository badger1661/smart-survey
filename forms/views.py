...
def view_form(request, form_id=None):
    ...
    if form_id == None:
        ...
        if request.user.is_teacher() and request.user == form_.teacher:
...
def view_replies(request, form_id = None):
    ...
    if request.user.is_teacher() and request.user == form.teacher:
...
class reply(LoginRequiredMixin, View):
    ...
    def post(self, request, form_id=None):
        ...
        #GETS THE ANSWERS ENTERED ON THE FRONTEND
...
def compare_form(request, form_one = None, form_two = None):
    ...
    if request.user.is_teacher() and request.user == form_one.teacher and request.user == form_two.teacher:
...
def remind(request):
    #GETS IDS OF STUDENTS WHO HAVENT RESPONDED
    if request.user.is_teacher():
        ...
        #GETS EACH STUDENTS USER OBJECT
        students = []
        for id_ in ids:
            try:
                ...
                pass
        #SENDS AN EMAIL TO EACH STUDENT REMINDING THEM TO REPLY
